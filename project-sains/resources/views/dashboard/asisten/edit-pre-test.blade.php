...
@section('title', 'SAINS - Edit Pre-Test')

@section('content')

<div class="container mx-auto px-4 min-h-screen">
    <h1 class="my-10 font-bold text-center md:text-2xl">Edit Nilai Pre-Test</h1>
    
    <form action="{{ route('pretest.updateAll', ['courseId' => $courseId]) }}" method="POST">
        @csrf
        @method('PUT')   
            ...
            <table class="w-full text-sm text-left p-3 rtl:text-right text-primary bg-gray-50">
                ...
                <tbody>
                    @foreach ($students as $student)
                        @php
                            $preTest = $preTestData[$student->user_id] ?? null;
                        @endphp
                        <tr class="border-b">
